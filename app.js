...
app.get(['/', '/home'], async function(req, res){
    ...
    res.send(header.header + nav.nav(seasonNav) + current.current(seasonsList.seasons.length, currentResults, seasonsList.seasons[seasonsList.seasons.length - 1].events.length) + news.news(newses) + about.about  + participant.participant(parts) + seasons.seasons + event.eventBlock + footer.footer)
})
...
app.get('/profile', async function(req, res){
    ...
    participants.participants[participants.participants.findIndex(prt => prt.id === req.query.name)].links.forEach(el => {
        ...
        if(el.linkName.indexOf('twitch') >= 0) {
    ...
    seasonResult.forEach(el => {
        if(el.playerName === req.query.name) {
            if(values.indexOf(el.seasonPoints)+1 === 1) {
                profileInfo += `
                    <div class="profile-info-achivements__item">
                        <div class="profile-info-achivements__item-medal">
                            <?xml version="1.0" encoding="utf-8"?><!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
                            <svg width="48px" height="48px" viewBox="0 0 128 128" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" class="iconify iconify--noto" preserveAspectRatio="xMidYMid meet">
                        ...
                        <p>${values.indexOf(el.seasonPoints)+1} место</p>
                        <p>${el.seasonPoints}</p>
                    </div>
                `
            }

            if(values.indexOf(el.seasonPoints)+1 === 2) {
                profileInfo += `
                    <div class="profile-info-achivements__item">
                        <div class="profile-info-achivements__item-medal">
                            <?xml version="1.0" encoding="utf-8"?><!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
            ...
            if(values.indexOf(el.seasonPoints)+1 === 3) {
                profileInfo += `
                    <div class="profile-info-achivements__item">
                        <div class="profile-info-achivements__item-medal">
                            <?xml version="1.0" encoding="utf-8"?><!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
            ...
            if(values.indexOf(el.seasonPoints)+1 === 4 || values.indexOf(el.seasonPoints)+1 === 5) {
                profileInfo += `
                    <div class="profile-info-achivements__item">
                        <p>${el.seasonNumber} сезон</p>
                        <p>${values.indexOf(el.seasonPoints)+1} место</p>
    ...
    res.send(header.header + profile.profile(req.query.name, profileInfo))
